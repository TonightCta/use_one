...
<template>
    <P-main class="login">
        ...
        <h3>{{$t('请输入')}}{{ title.VerificationName }}{{$t('验证码')}}
        ...
        <div class="getVerificationCode ui-flex-center">
            ...
            <div>
                <div @click="getVerificationCode" v-if="count == 0">{{$t('获取验证码')}}</div>
                <B-data-theCountdown v-model="count" v-else>{{count}} {{$t('秒后重新获取')}}</B-data-theCountdown>
            </div>
        </div>
    </P-main>
</template>
...
export default {
    ...
    methods: {
        getVerificationCode() {
            this.count = 60;
            this.$store.dispatch('registered/sendCode', {
                scene: this.scene,
                type: this.title.id,
                email: this.email,
                phone: this.phone,
