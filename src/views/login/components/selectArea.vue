...
<template>
    <P-main v-if="value">
        <Navigation :searchSwitch="true" v-model="keyword" :show-action="true" :left-arrow="false" />

        <div v-if="keyword == ''">
            <van-cell-group title="热门地区" class="m-30-b">
                <van-cell v-for="(item, index) in hot" :key="index" @click="selected(item)">
                    <template #title>
        ...
        <van-cell-group v-else>
            <van-cell v-for="(item, index) in setData" :key="index" @click="selected(item)">
                <template #title>
                    <div class="ui-flex-left">
                        <img :src="item.logo" class="theNationalFlag">
                        <div>{{item.country}}</div>
